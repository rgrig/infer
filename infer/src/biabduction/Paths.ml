...
module Path : sig
  ...
  let get_path_ids pname path : (int ProcPathMap.t * (unit -> int)) =
    ...
    let add pname path (ids : (int PathMap.t * int NodeMap.t) ProcMap.t) =
      ...
      let by_path, by_node =
        ...
        with Caml.Not_found -> (PathMap.empty, NodeMap.empty) in
      let i =
        if is_coalescing then
          (try NodeMap.find (node_of path) by_node
          with Caml.Not_found -> next_id ())
        else next_id () in
  ...
  let get_calls ?(coalesce = true) ?(with_epsilon = false) pname path =
    ...
    let i pname path =
      ...
      with Caml.Not_found -> L.(die InternalError) "Path without ID." in
    let s pname =
      ...
      (fun p -> (
        try
          ...
          i pname path
        with Caml.Not_found -> L.(die InternalError) "Path without a start.")) in
